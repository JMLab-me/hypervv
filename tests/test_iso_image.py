...
def test_generate_iso(tmp_path_factory : TempPathFactory):
    maps = [
        IsoFileMap(
            file_dep=Path("tests/assets/iso/file1.txt"),
            iso_mapping=Path("/directory/file1.txt")),
        IsoFileMap(
            file_dep=Path("tests/assets/iso/nested/nested_nested/nested_nested_file1.txt"),
            iso_mapping=Path("/directory/nested_nested_file1.txt")
        ),
        IsoFileMap(
            file_dep=Path("tests/assets/iso/nested/nested_nested/nested_nested_file2.txt"),
    ...
    generate_iso(maps, deps, [str(output.resolve())])
